...
ask the caller to resolve some choice (`undefined`, an access to memory
outside `dmem`, or an access permission) before continuing, or hand a
final `MachineData` to the instruction's `next`/`jmp` continuation.

`Effects.PreservesOutside post` says "for every way of resolving those
choices, the state eventually reached satisfies `post`". It resembles
Kraken's own `Effects.All` (`Kraken.X64.OmniSemantics`), but treats an
access outside `dmem` (`nonmem_load`/`nonmem_store`) the same as an
`undefined` choice rather than as failure: `written_regs`/`modifies_flags`
are claims purely about registers and flags, which such an access cannot
affect (it only ever changes `dmem`, see `MachineData.load`/`.store`), so
there is no need to rule it out here the way a full correctness proof
would. -/
def Effects.PreservesOutside (post : MachineData → Prop) : Effects → Prop
  | .done a => post a.1
  | .unimplemented _ => True
  | .gp_unaligned .. => True
  | .nonmem_load _ _ _ ret => ∀ v dmem, PreservesOutside post (ret v dmem)
  | .nonmem_store _ _ _ ret => ∀ dmem, PreservesOutside post (ret dmem)
  | @Effects.undefined α _ ret => ∀ v : α, PreservesOutside post (ret v)
  | .require_read_access _ _ ok => PreservesOutside post (ok ())
  | .require_write_access _ _ ok => PreservesOutside post (ok ())
  | .require_exec_access _ ok => PreservesOutside post (ok ())
