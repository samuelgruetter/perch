...
/-
Everything until here (i.e. only minimal standalone example) works as expected.
But when I try the same in a bigger context, I get a typechecking error that
`first` does not discard, even though it could just fall through to the `sorry`
as the last option.
-/
